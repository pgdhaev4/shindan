...
class QuizEngine {
  ...
  _saveResult(key, result) {
    try {
      ...
      if (!slug) return;
      localStorage.setItem('wp_' + slug, key);
      localStorage.setItem('wp_' + slug + '_name', result.name || '');
  ...
  _statToPercent(val) {
    ...
    // 100% - 最大・完全・消滅済み
    if (/∞|MAX|SSS|神|9999|化け物|全員|完了|常時開放|核融合|削除済み|光速/.test(v)) return 100;
    // 93% - 非常に高い
    if (/高い|異常|鋼鉄|ダイヤ|^高$|フロア全体/.test(v)) return 93;
    // %表記
    ...
    // ~4% - 存在しない・検出不能
    if (/^なし$|検出不能|検知不能/.test(v)) return 4;
    // ~18% - 低い・薄い・不明
    if (/低下中|低い|少なめ|遅い|極小|不明|^低$|概念として/.test(v)) return 18;
    // ~50% - 普通・運次第・未計測
    if (/普通|気分次第|運次第|未計測/.test(v)) return 50;
    // ~35% - あるはずだが消えかけている
